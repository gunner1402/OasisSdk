...
//      FlashlightManager.disableFlashlight();
  ...
  public Rect getFramingRect() {
    ...
    if (framingRect == null) {
      ...
      if(width < height){// 获取较小的值，绘制扫描区域
    	  height = width;
      }else
    	  width = height;
      
      if(screenResolution.x > screenResolution.y){// 横屏
//    	  int leftOffset = (screenResolution.x) / 2 + ((screenResolution.x) / 2 - width) / 2;// 屏幕右半边
    	  int leftOffset = ((screenResolution.x) / 2 - width) / 2;// 屏幕左半边
          int topOffset = (screenResolution.y - height) / 2;//  - height 定位在屏幕右半区，并居中
          framingRect = new Rect(leftOffset, topOffset, leftOffset + width, topOffset + height);
          Log.d(TAG, "Calculated framing rect: " + framingRect);
          return framingRect;
      }
      int leftOffset = (screenResolution.x - width) / 2;
      int topOffset = (screenResolution.y) / 2;//  - height 定位在屏幕下半区，并居中
      framingRect = new Rect(leftOffset, topOffset, leftOffset + width, topOffset + height);
      Log.d(TAG, "Calculated framing rect: " + framingRect);
    }
    return framingRect;
